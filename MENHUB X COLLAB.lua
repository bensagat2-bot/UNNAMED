...
-- Hash variables and checks are removed. The script will no longer
-- compare hashes and will not kick on hash mismatch.
...
getgenv().config = {
    ...
    -- Ragebot settings (from SkidOnyx)
    ...
    ragebot_primary = "AssaultRifle",
    ragebot_secondary = "Revolver",
    ragebot_melee = "Knife",
}
...
-- ================================================================
--                    OBSIDIAN UI (Original)
-- ================================================================

local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/deividcomsono/Obsidian/main/Library.lua"))()
local Window = Library:CreateWindow({
    Title = "MENHUB X COLLAB",   -- Changed UI name
    Center = true,
    AutoShow = true,
    Resizable = true,
...
-- ================================================================
--                    FEATURE FUNCTIONS (MENHUB ORIGINAL)
-- ================================================================
...
-- ================================================================
--        OPTIMIZED SLINGSHOT RAGE (Event-driven, no lag)
--        WITH MINIMUM TRAVEL TIME FIX FOR TELEPORT + PREDICTION
-- ================================================================
local slingProjs = {}
        ...
        if enemy and enemy.Character and myHrp then
            ...
            if hrp then
                ...
                if config.slingPrediction then
                    ...
                    local travelTime = math.max(dist / math.max(config.slingProjSpeed, 1), 0.05)
...
local RagebotTab  = Window:AddTab("Ragebot")   -- New Ragebot tab
...
CfgBox:AddButton("Reset Config to Defaults", function()
    getgenv().config = {
        ...
        ragebot_enabled = false,
        target = {
            enabled = false, character = nil, auto = false, autoshoot = true, shootAttempts = 1, hitpart = "Head",
            lastchar = nil, lastplayer = nil, manualkey = false, immune = false, attackPosition = "default",
            attackCustomEnabled = false, weaponPick = "primary", forceWeapon = true, rageMasterOn = false,
            rightClickKnife = false, meleeFeetDrop = 3.5, underOffset = 4, customHeight = 2, customFront = 0,
            customSide = 0, customVertical = 0, customRadius = 0, targetSort = "nearest", teamCheck = true,
            wallCheck = false, maxDistance = 0, autoSwitch = true, safeHP = 0, holdKey = false,
        },
        prediction = { enabled = false, multiplier = 1.2, velocity = Vector3.new(0,0,0), lastposition = nil, lasttime = 0 },
        orbit = { active = false, angle = 0, serverpos = nil, connection = nil, speed = 9000, orbitSpeed = 9000,
                  height = 1, radius = 0, originalpos = nil },
        rage = { enabled = false, evasion = "Random", evasionRadius = 1e14, evasionRandomRange = 0.5,
                 evasionTranslocateOffset = 0, shootFrames = 1, hitboxWindow = 0.625, attackCooldown = 1.25,
                 defensiveCFrame = true },
        state = { reloading = false, outofammo = false, csyncactive = false, reloadStartedAt = 0 },
        voidhide = { enabled = true, originalPosition = nil, active = false, connection = nil },
        voidspam = { enabled = false, shoot_min = 1, shoot_max = 1, hide_min = 1, hide_max = 1, phase = nil,
                     lastswitch = 0, currentduration = 0, bypassMode = "Extreme Networking" },
        backstab = { enabled = false, camera = false, hitboxWindowUntil = -1, attackCooldownUntil = -1 },
        visualizer = { enabled = true, tracer = { color = Color3.fromRGB(0,186,255), thickness = 1, transparency = 1,
                     start_point = "cursor", outline = true, outline_color = Color3.fromRGB(0,0,0),
                     outline_thickness = 1 },
                     indicator = { display_options = {"name","position","hit reg"}, color = Color3.fromRGB(255,255,255),
                     accent_color = Color3.fromRGB(0,186,255) } },
        hitNotifications = { enabled = true, color = Color3.fromRGB(235,235,235), textSize = 14, maxVisible = 8,
                             duration = 3, stackGap = 6, position = "Top Left", offsetX = 12, offsetY = 12,
                             inAnimation = "fade bounce", outAnimation = "fade", animInDuration = 0.52,
                             animOutDuration = 0.38, uiNotif = false },
        ragestatus = { enabled = false, mode = "static", color = Color3.fromRGB(235,235,235), staticOffsetX = 0,
                       staticOffsetY = 40, showAmmo = true, hideOnReload = true, textSize = 15, lineGap = 15,
                       fontName = "gotham", showKills = true },
        kills = { count = 0, streak = 0, lastKillAt = 0 },
        ragebot_primary = "AssaultRifle",
        ragebot_secondary = "Revolver",
        ragebot_melee = "Knife",
    }
...
-- ================================================================
--                    RAGEBOT IMPLEMENTATION (Rivals-compatible)
-- ================================================================
...
function Ragebot.new()
    ...
    self._connections = {}
    self._renderStepName = "RagebotAntiAim"
    ...
    -- Main loop for void spam attack
    self._heartbeatConn = RunService.Heartbeat:Connect(function()
        if config.ragebot_enabled and config.voidspam.enabled then
    ...
    -- Auto attack loop (no void required)
    task.spawn(function()
        while true do
            if config.ragebot_enabled and config.target.autoshoot then
                self:autoAttackLoop()
            end
            task.wait(0.1)
        end
...
function Ragebot:setupAntiAim()
    ...
    local function antiAimStep()
        if not config.ragebot_enabled then
            self:unbindAntiAim()
            return
        end
        ...
        self.desync.enabled = config.anti_aim_enabled or config.rage.desync
        self.desync.pitch = config.anti_aim_pitch or 0
        self.desync.yaw = config.anti_aim_yaw or 0
        self.desync.customPitch = config.anti_aim_custom_pitch or 0
        self.desync.customYaw = config.anti_aim_custom_yaw or 0
        self.desync.floorHide = config.anti_aim_floor_hide or false
    ...
    RunService:BindToRenderStep(self._renderStepName, Enum.RenderPriority.Camera.Value, antiAimStep)
    table.insert(self._connections, {Disconnect = function() self:unbindAntiAim() end})
end
...
function Ragebot:setupVoidSpam()
    ...
    -- Priority weapon getter
    function self:getBestWeapon()
        local myChar = localplayer.Character
        if not myChar then return nil end

        -- Get all tools (equipped + in backpack)
        local tools = {}
        local equipped = myChar:FindFirstChild("EquippedItem") or myChar:FindFirstChildOfClass("Tool")
        ...
        -- Priority order: primary, secondary, melee
        local priorityNames = {
            config.ragebot_primary,
            config.ragebot_secondary,
            config.ragebot_melee
        }

        for _, desiredName in ipairs(priorityNames) do
            if desiredName and desiredName ~= "" then
                for _, tool in ipairs(tools) do
                    -- Case-insensitive partial match (Rivals weapon names)
                    if tool.Name:lower():find(desiredName:lower()) then
                        -- Check ammo if gun (Rivals uses "Ammo" or "CurrentAmmo")
                        local data = tool:FindFirstChild("Data")
                        local ammo = data and (data.Ammo or data.CurrentAmmo)
                        if ammo and ammo.Value > 0 then
                            return tool
                        elseif not ammo then
                            -- Melee or no ammo system
                            return tool
                        end
                    end
                end
        ...
        -- Fallback: return any tool
        return tools[1]
    end

    -- Reliable attack function for Rivals
    function self:fireWeapon(tool, target)
        if not tool or not target then return false end

        -- Try remote attack first (Rivals style)
        if UseItemRemote and Utility and self:getHitbox then
            ...
            if hitbox then
                ...
                local payload = {
                    ...
                    item = tool.Name,
                    position = aimPos,
                    attackNum = self.attackNum + 1,
                    animationKeys = {},
                    rotation = CFrame.new().Rotation,
                    one = Vector3.one,
                    packed = { "\x00", "\x01" },
                }
        ...
        -- Fallback 2: mouse click simulation
        pcall(function()
            if UserInputService.TouchEnabled then
                local cam = workspace.CurrentCamera
                if cam then
                    local center = cam.ViewportSize / 2
                    if firetouchtap then firetouchtap(center) end
                end
            else
    ...
    function self:voidLoop()
        if not config.ragebot_enabled or not config.voidspam.enabled then return end
        ...
        local item = self:getBestWeapon()
        if not item then
            self.status.text = "no usable weapon"
            return
        end
        -- If item is not equipped, equip it
        if item.Parent ~= myChar then
            pcall(function()
                local eq = item:FindFirstChild("Equip")
                if eq and eq:IsA("RemoteEvent") then
                    eq:FireServer()
                else
                    item.Parent = myChar
                end
        ...
        self:fireWeapon(item, target)
        self.status.text = "void spamming " .. target.Name
    end

    -- Auto attack loop (no void)
    function self:autoAttackLoop()
        while config.ragebot_enabled and config.target.autoshoot do
            task.wait(0.1)
            local target = self:getPriorityTarget()
            if not target then
                self.status.text = "no target"
                continue
            end
            local myChar = localplayer.Character
            if not myChar then continue end

            local tool = self:getBestWeapon()
            if not tool then
                self.status.text = "no weapon"
                continue
            end
            -- Equip if not already
            if tool.Parent ~= myChar then
                pcall(function()
                    local eq = tool:FindFirstChild("Equip")
                    if eq and eq:IsA("RemoteEvent") then
                        eq:FireServer()
                    else
                        tool.Parent = myChar
            ...
            -- Attack!
            self:fireWeapon(tool, target)
            self.status.text = "attacking " .. target.Name
        end
    end

    -- RenderStepped for status update
    local conn = RunService.RenderStepped:Connect(function()
        if not config.ragebot_enabled then return end
        if not config.voidspam.enabled and not config.target.autoshoot then return end
        local myChar = localplayer.Character
        if not myChar then return end
        local item = self:getBestWeapon()
        if not item then
            self.status.text = "no usable item(s)"
            return
        end
...
function Ragebot:setupPriorityTargeting()
    ...
    function self:addPriority(player, level)
        ...
        if config.ragebot_priority_notification then
            Notify("Priority", "added " .. player.Name .. " to ragebot priority")
        end
    end
...
function Ragebot:setupIndicator()
    ...
    local conn = RunService.RenderStepped:Connect(function()
        ...
        self.status.enabled = config.ragebot_enabled
        ...
        if weaponType == "Gun" then
            self.status.text = "ragebot (gun)"
            self.status.weapon = item.Name
        elseif weaponType == "Melee" then
            self.status.text = "ragebot (melee)"
            self.status.weapon = item.Name
        else
            self.status.text = "invalid weapon"
...
function Ragebot:Destroy()
    ...
    self:unbindAntiAim()
    if self.attackThread and coroutine.status(self.attackThread) ~= "dead" then
        coroutine.close(self.attackThread)
    end
    self._connections = {}
end
...
local RageGeneralBox = RagebotTab:AddLeftGroupbox("General")
Tgl(RageGeneralBox, "RagebotEnable", "Ragebot Enabled", "Master toggle for ragebot.", function(v)
    config.ragebot_enabled = v
end)
Tgl(RageGeneralBox, "AutoAttack", "Auto Attack", "Automatically attack nearest enemy (no void).", function(v)
    config.target.autoshoot = v
end)
Tgl(RageGeneralBox, "VoidSpamAttack", "Void Spam Attack", "Attack while teleporting into void (chaotic).", function(v)
    config.voidspam.enabled = v
end)

local AntiAimBox = RagebotTab:AddLeftGroupbox("Anti-Aim (Desync)")
Tgl(AntiAimBox, "AntiAimEnable", "Anti-Aim Enabled", "Enable desync/anti-aim.", function(v)
    config.anti_aim_enabled = v
end)
Tgl(AntiAimBox, "AntiAimFloorHide", "Floor Hide (Voidspam Hide)", "Move under map when low.", function(v)
    config.anti_aim_floor_hide = v
end)
AntiAimBox:AddSlider("AAPitch", { Text = "Pitch", Default = 0, Min = -180, Max = 180, Rounding = 0, Callback = function(v) config.anti_aim_pitch = v end })
AntiAimBox:AddSlider("AAYaw", { Text = "Yaw", Default = 0, Min = -180, Max = 180, Rounding = 0, Callback = function(v) config.anti_aim_yaw = v end })
AntiAimBox:AddSlider("AACustomPitch", { Text = "Custom Pitch", Default = 0, Min = -180, Max = 180, Rounding = 0, Callback = function(v) config.anti_aim_custom_pitch = v end })
AntiAimBox:AddSlider("AACustomYaw", { Text = "Custom Yaw", Default = 0, Min = -180, Max = 180, Rounding = 0, Callback = function(v) config.anti_aim_custom_yaw = v end })

local WeaponBox = RagebotTab:AddRightGroupbox("Weapon Priority")
WeaponBox:AddInput("RagePrimary", {
    Text = "Primary Weapon",
    Default = "AssaultRifle",
    Numeric = false,
    Finished = false,
    Callback = function(v) config.ragebot_primary = v end
})
WeaponBox:AddInput("RageSecondary", {
    Text = "Secondary Weapon",
    Default = "Revolver",
    Numeric = false,
    Finished = false,
    Callback = function(v) config.ragebot_secondary = v end
})
WeaponBox:AddInput("RageMelee", {
    Text = "Melee Weapon",
    Default = "Knife",
    Numeric = false,
    Finished = false,
    Callback = function(v) config.ragebot_melee = v end
})
WeaponBox:AddButton("Refresh Weapon List", function()
    local list = {}
    local backpack = localplayer.Backpack
    if backpack then
        for _, child in ipairs(backpack:GetChildren()) do
    ...
    if #list > 0 then
        Notify("Weapons Found", table.concat(list, ", "))
    else
        Notify("Weapons", "Check your inventory (Backpack).")
    end
end)

local PriorityBox = RagebotTab:AddRightGroupbox("Priority Targeting")
PriorityBox:AddButton("Add Closest as Priority", function()
    local target = getClosest()
    if target then
        ragebot:addPriority(target, 10)
        Notify("Priority", "Added " .. target.Name .. " with priority 10")
    else
        Notify("Priority", "No target found")
    end
end)
PriorityBox:AddButton("Clear Priorities", function()
    for player in pairs(ragebot.priorities) do
        ragebot:removePriority(player)
    end
    Notify("Priority", "Cleared all priorities")
end)
PriorityBox:AddButton("List Priorities", function()
    local list = ragebot:getPriorityList()
    if #list == 0 then Notify("Priority", "No priorities set.")
    else
        local str = {}
        for _, entry in ipairs(list) do
            table.insert(str, entry.player.Name .. " (" .. entry.priority .. ")")
        end
        Notify("Priority", table.concat(str, ", "))
    end
end)
...
Notify("MENHUB X COLLAB", "Loaded successfully (Obsidian UI, optimized Slingshot Rage + Full Ragebot for Rivals)")